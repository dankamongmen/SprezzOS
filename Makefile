...
$(IMG): $(MAKECD) $(CONF) $(SEED) $(PROFILE) $(CHROOT)/$(DIDEB) $(FONT)
...
$(SEED): $(SEEDIN)
	@[ -d $(@D) ] || mkdir -p $(@D)
	( echo "# Automatically generated by Make" && cat $^ && echo "d-i base-installer/kernel/which-kernel select linux-image-$(KVER)-amd64" ) > $@
